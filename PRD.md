...
|-----------|-------------|---------------------|
| 专注计时页 | 计时器模块 | 显示当前倒计时，支持自定义时长（15-120分钟），开始/暂停/重置/放弃操作；专注成功：倒计时正常结束获得全额小鱼干；专注失败：点击重置、暂停超过3分钟或点击放弃按钮，无法获得奖励；暂停提醒：暂停超过1分钟时小猫会以弹窗或动画提醒用户继续专注 |
...
## 3. Core Process
用户主要使用流程：打开应用 → 进入专注计时页设置时长 → 开始专注 → （可能暂停）→ 专注成功获得小鱼干或专注失败无奖励 → 进入宠物主页互动或商店购买装饰 → 查看统计数据
...
graph TD
    ...
    C --> D[开始专注计时]
    D --> E{用户操作?}
    E -->|倒计时正常结束| F[专注成功<br/>获得全额小鱼干]
    E -->|点击重置| G[专注失败<br/>无奖励]
    E -->|点击放弃| G
    E -->|点击暂停| H{暂停时长?}
    H -->|≤1分钟| I[继续计时]
    H -->|>1分钟且≤3分钟| J[小猫弹窗提醒]
...
|-----------|-------------|-------------|
| 专注计时页 | 计时器模块 | 大字号倒计时显示，周围有柔和的光晕动画，背景采用图片附件中的温馨手绘风格，底部有开始/暂停/重置/放弃按钮；暂停时显示暂停时长；小猫提醒弹窗采用可爱的气泡样式，带有小猫表情和鼓励文字 |
